...
void Cmd_CritInf_f(const idCmdArgs& args) {
	idPlayer* player;

	player = gameLocal.GetLocalPlayer();
	if (!player) return;
	int rate = int(100 * player->crit_rate);
	int dmg = int(100 * player->crit_dmg);
	gameLocal.Printf("Crit rate is %i and crit damage is %i", rate, dmg);
}
#ifndef _FINAL
...
// RITUAL END
	cmdSystem->AddCommand("critinf",				Cmd_CritInf_f,				CMD_FL_GAME,				"");
